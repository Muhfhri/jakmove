self.onmessage = (ev) => {
	...
	try {
		post('progress', { percent: 88, status: 'Mem-parse CSV: stops...' });
		const stops = parseCSV(texts.stopsTxt || '');
		post('progress', { percent: 89, status: 'Mem-parse CSV: routes...' });
		const routes = parseCSV(texts.routesTxt || '');
		post('progress', { percent: 90, status: 'Mem-parse CSV: trips...' });
		const trips = parseCSV(texts.tripsTxt || '');
		post('progress', { percent: 91, status: 'Mem-parse CSV: stop_times...' });
		const stop_times = parseCSV(texts.stopTimesTxt || '');
		post('progress', { percent: 92, status: 'Mem-parse CSV: shapes...' });
		const shapes = parseCSV(texts.shapesTxt || '');
		post('progress', { percent: 93, status: 'Mem-parse CSV: lainnya...' });
		...
		post('progress', { percent: 94, status: 'Membangun indeks halte→rute...' });
		const stopToRoutes = buildStopToRoutes(stop_times, trips);

		post('progress', { percent: 96, status: 'Finalisasi data...' });
		post('result', { data: { stops, routes, trips, stop_times, shapes, frequencies, fare_rules, fare_attributes, transfers, calendar, agency, stopToRoutes } });
	} catch (e) {
		post('error', { error: e && e.message ? e.message : 'Unknown error' });
	}
};
